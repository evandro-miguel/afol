...
| **What gets archived** | Finalized sessions (status: `final`, `draft`, `deprecated`) only | Active sessions must stay in `.agents/wb/` |
| **Archive trigger** | When root `.agents/wb/` has more than 3 finalized sessions | Keeps active surface low-noise for agents |
...
mv .agents/wb/<session-id> .agents/z-arq/<session-id>
