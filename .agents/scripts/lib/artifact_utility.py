...
TASK_TABLE_RE = re.compile(
    r"^\|\s*(T-\d{2,3})\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|$", re.MULTILINE
)
...
def _section_body(body: str, heading: str) -> str:
    pattern = re.compile(
        SECTION_RE_TEMPLATE.format(heading=re.escape(heading)), re.MULTILINE | re.DOTALL
    )
...
def _append_missing_section(
    reasons: List[str], body: str, heading: str, message: str, minimum: int = 1
) -> None:
...
def _plan_has_real_progress(body: str) -> bool:
    ...
    return any(
        "Replace this line" not in line and not PLACEHOLDER_RE.search(line)
...
def _check_brainstorm(body: str, reasons: List[str]) -> None:
    _append_missing_section(
        reasons, body, "Problem Statement", "problem statement has no concrete content"
    )
    _append_missing_section(
        reasons, body, "Options", "options section has no concrete alternatives"
    )
    _append_missing_section(
        reasons, body, "Preferred Direction", "preferred direction has no concrete rationale"
    )
...
def _check_explorer_check(body: str, reasons: List[str]) -> None:
    _append_missing_section(
        reasons, body, "Scope Reviewed", "scope reviewed has no inspected paths or docs"
    )
    _append_missing_section(
        reasons, body, "Findings", "findings section has no repo-backed findings"
    )
    _append_missing_section(
        reasons, body, "Impact on the Plan", "impact on the plan has no concrete consequence"
    )
...
def _check_plan(body: str, reasons: List[str]) -> None:
    ...
    _append_missing_section(
        reasons, body, "Concrete Steps", "concrete steps section has no actionable steps"
    )
    _append_missing_section(
        reasons,
        body,
        "Validation and Acceptance",
        "validation and acceptance has no executable checks",
    )
...
def _check_report(body: str, reasons: List[str]) -> None:
    ...
    _append_missing_section(
        reasons, body, "Delivered Changes", "delivered changes has no concrete change list"
    )
    _append_missing_section(
        reasons, body, "Verification", "verification has no concrete command/result evidence"
    )


def _check_postmortem(body: str, reasons: List[str]) -> None:
    _append_missing_section(
        reasons, body, "What Was Achieved", "what was achieved has no concrete outcome"
    )
    _append_missing_section(reasons, body, "Root Causes", "root causes has no concrete analysis")
    _append_missing_section(
        reasons, body, "Follow-ups for Next Rounds", "follow-ups has no concrete next step"
    )
...
def analyze_artifact_utility(
    doc_type: str, body: str, frontmatter: Dict[str, Any] | None = None
) -> Dict[str, Any]:
