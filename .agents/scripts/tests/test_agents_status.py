...
class StatusTests(unittest.TestCase):
    ...
    def test_main_returns_int_exit_code(self):
        ...
        mock_args = mock.Mock(
            session=None, json=False, pretty=False, artifact=[], check_context=False
        )
        with (
...
class MainPathTests(unittest.TestCase):
    ...
    def test_main_json_output(self):
        mock_args = mock.Mock(
            session=None, json=True, pretty=False, artifact=None, check_context=False
        )
        with (
            mock.patch.object(self.status, "parse_args", return_value=mock_args),
            mock.patch.object(self.status, "find_session", return_value=Path("/tmp/s")),
    ...
    def test_main_artifact_resolution(self):
        mock_args = mock.Mock(
            session=None, json=False, pretty=False, artifact=["task", "plan"], check_context=False
        )
        with (
            mock.patch.object(self.status, "parse_args", return_value=mock_args),
            mock.patch.object(self.status, "find_session", return_value=Path("/tmp/s")),
    ...
    def test_main_error_path(self):
        mock_args = mock.Mock(
            session=None, json=False, pretty=False, artifact=None, check_context=False
        )
        with (
            mock.patch.object(self.status, "parse_args", return_value=mock_args),
            mock.patch.object(
    ...
    def test_main_rejects_pretty_without_json(self):
        mock_args = mock.Mock(
            session=None, json=False, pretty=True, artifact=None, check_context=False
        )
        with (
            mock.patch.object(self.status, "parse_args", return_value=mock_args),
            mock.patch.object(self.status, "find_session", return_value=Path("/tmp/s")),
