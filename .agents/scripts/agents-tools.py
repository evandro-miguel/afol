...
def search_tools(tools_data: Dict[str, Any], query: str) -> None:
    ...
    for tool in tools:
        ...
        if any(term in normalize_text(tool.get("name", "")) for term in terms):
            score += 4
            matched_fields.append("name")

        # Search in type
        if any(term in normalize_text(tool.get("type", "")) for term in terms):
            score += 2
            matched_fields.append("type")

        # Search in command names/usages
        for cmd in tool.get("commands", []):
            if isinstance(cmd, dict):
                ...
                cmd_text = str(cmd)
            if any(term in normalize_text(cmd_text) for term in terms):
                score += 2
