...
- Why now: the scaffold already has roadmap/spec/workbench governance, but the behavior of bootstrap, status, review, and resolver commands does not yet explicitly reinforce the new taxonomy between `arc/map/` and goal-state docs.
...
- Status and review flows need to know when to point users at `arc/map/` for evidence and when to point them back to roadmap, specs, architecture, or the active workbench session.
...
5. Resolver behavior helps route questions to either `arc/map/` evidence or goal-state docs, depending on whether the user needs current state or intended state.
...
- Expected behavior:
  ...
  - Workbench sessions remain the place for execution narrative, while `arc/map/` remains evidence-only.
...
- Assumptions:
  - `.agents/arc/GENERAL-ROADMAP.md` and `SPECS/` remain the canonical governance layer.
  - `.agents/wb/` remains the canonical place for execution artifacts.
  - `arc/map/` is evidence-oriented and may be absent in some repos until adopted.
- Constraints:
...
- keep workbench as the execution surface and `arc/map/` as the evidence surface
...
- updated bootstrap guidance at `.agents/scripts` and `.agents/a-docs`

Dependency rules:
- Allowed deps: bootstrap -> docs -> command guidance -> workbench -> roadmap/specs
- Forbidden deps: bootstrap -> new planning tree
- Forbidden deps: status/review/resolver -> approval authority over `arc/map/`
...
- Governance docs: `.agents/arc/` key: roadmap feature and spec id
- Current-state evidence: `.agents/arc/map/` key: repo-specific artifact name
...
5. `resolver` sends current-state questions to `arc/map/` and goal-state questions to roadmap/specs.

Error paths:
- E-01 Missing `arc/map/` surface -> bootstrap and status fall back to roadmap/spec/workbench guidance without fabricating a map tree
