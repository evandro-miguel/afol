...
## 1) Feature Intent
- Outcome: the scaffold defines the contract for `arc/map/` as a descriptive current-state surface for repository maps and analysis evidence.
...
- It does not yet define which map-like artifacts belong to `arc/map/`, how they are owned, or how often they are refreshed.
...
2. The maintainer stores those artifacts under `arc/map/` as descriptive evidence.
...
- Expected behavior:
  - `arc/map/` is descriptive, refreshable, and non-governance.
  ...
  - Workstreams may cite `arc/map/` as input evidence, but they must not treat it as the source of execution authority.
- Boundaries:
...
## 5) Scope
In scope:
- likely `arc/map/` artifact families and their purpose
...
- making `arc/map/` a required approval gate
...
- Assumptions:
  - `arc/map/` is project-specific and may vary by repository maturity.
...
## 8) Acceptance
- Success looks like:
  - Maintainers can tell which artifacts belong in `arc/map/` without guessing.
  - Agents can read current-state evidence from `arc/map/` without treating it as approval authority.
  - Goal-state docs outside `arc/map/` remain the canonical place for roadmap, specs, and architecture intent.
- Review questions:
...
- Risk: `arc/map/` becomes stale and misleading -> Mitigation: define it as refreshable evidence, not authoritative intent.
...
- Evidence expected from delivery:
  - documentation that names the `arc/map/` surface and its allowed artifact classes
