...
- Why now: downstream repos increasingly need richer map surfaces under `.agents/arc/map/`, but the scaffold does not yet define how those artifacts relate to roadmap, specs, architecture, and workbench planning.
...
2. The operator reads `arc/map/` for descriptive current-state evidence and current technical surfaces.
...
4. A workstream references roadmap/specs for execution authority and may cite `arc/map/` only as input evidence.
...
- Expected behavior:
  - `.agents/arc/map/` is reserved for current-state, descriptive, evidence-heavy artifacts.
  - Goal-state docs remain outside `arc/map/` and continue to define product intent, architecture direction, roadmap features, and spec boundaries.
  - Workstreams may inspect `arc/map/` during brainstorm, explorer-check, research, review, and status flows, but must still link to roadmap/spec governance for approved intent.
...
- Boundaries:
  ...
  - This feature does not require every project to generate the full `arc/map/` surface immediately.

## 5) Scope
In scope:
- the document taxonomy for current-state vs goal-state artifacts
- semantic boundaries for `arc/map/` and the rest of `.agents/arc/`
...
- Child specs:
  - `260323_1750_current-state-map-contract_spec_01` -> define required/optional `arc/map/` surfaces and ownership
  - `260323_1751_goal-state-canon_spec_01` -> define which docs outside `arc/map/` remain strategic and normative
  - `260323_1752_workflow-and-bootstrap-integration_spec_01` -> define command/docs/bootstrap behavior for the split

## 7) Constraints and Assumptions
- Assumptions:
  - `.agents/arc` remains the canonical home for strategic project context.
  - `arc/map/` artifacts may be partially generated, partially curated, and more refreshable than roadmap/spec docs.
- Constraints:
...
- Success looks like:
  ...
  - Downstream projects can adopt `arc/map/` without weakening roadmap-first governance.
...
- Risk: maintainers create duplicate architecture docs inside and outside `arc/map/` -> Mitigation: define clear boundaries and ownership per doc class.
...
- Backout or deferral:
  - projects can continue without `arc/map/` until the contract and tooling are ready, but must preserve the goal-state canon outside that folder
...
- Open questions:
  - Q-01 Which `arc/map/` artifacts should be mandatory, optional, or generated on demand?
