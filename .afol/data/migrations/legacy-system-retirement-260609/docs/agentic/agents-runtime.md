...
- Legacy runtime archive operations remain factory-only compatibility behavior.
- AFOL provider-compatible cleanup archives mutable legacy state under `.afol/data/migrations/<timestamp>/` instead of writing new migration artifacts into `.agents/`.
