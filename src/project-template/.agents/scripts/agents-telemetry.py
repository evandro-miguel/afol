...
TELEMETRY_DATA_DIR = Path(
    os.environ.get(
        "AGENTS_TELEMETRY_DATA_DIR",
        str(Path(__file__).parent.parent / "data" / "telemetry"),
    )
)
TELEMETRY_SCHEMA_PATH = Path(
    os.environ.get(
        "AGENTS_TELEMETRY_SCHEMA_PATH",
        str(TELEMETRY_DATA_DIR / "schemas" / "event.json"),
    )
)
TELEMETRY_EVENTS_FILE = Path(
    os.environ.get(
        "AGENTS_TELEMETRY_EVENTS_FILE",
        str(TELEMETRY_DATA_DIR / "events.jsonl"),
    )
)
...
def _collect_element_stats(events: List[Dict], period_start: datetime) -> Dict[str, Dict]:
    ...
    for event in events:
        ...
        if timestamp is None:
            continue
        element_id, element_type = _extract_element_identity(event_type, metadata)
        if not element_id:
            continue
        ...
        if timestamp < period_start:
            continue
        _update_element_stats(element_stats, key, element_id, element_type, outcome, days_ago)
...
def _extract_element_identity(
    ...
    if event_type == "pattern_applied":
        return metadata.get("pattern_id"), "pattern"
    if event_type in ["element_access", "element_view", "element_apply"]:
        return metadata.get("element_id"), metadata.get("element_type", "document")
    return None, None


def _update_element_stats(
...
def main():
    ...
    command_handlers = {
        "record": lambda: _handle_record(args),
        "query": lambda: _handle_query(args),
        "export": lambda: _handle_export(args),
        "report": lambda: _handle_report(args),
        "validate": _handle_validate,
        "heat": lambda: _handle_heat(args),
        "hot": lambda: _handle_hot(args),
        "cold": lambda: _handle_cold(args),
    }
...
def _handle_export(args: argparse.Namespace) -> None:
    export_events(output_path=args.output, output_format=args.output_format)
...
def _handle_validate() -> None:
    success = validate_telemetry()
    sys.exit(0 if success else 1)


def _heat_elements(heat_data: Dict[str, Any], element_type: str) -> List[Dict[str, Any]]:
    return heat_data.get(element_type, heat_data.get("all", []))


def _handle_heat(args: argparse.Namespace) -> None:
...
def _handle_cold(args: argparse.Namespace) -> None:
    heat_data = calculate_heat_scores(period=args.period)
    elements = _heat_elements(heat_data, args.type)
...
def print_heat_elements(
    elements: List[Dict[str, Any]], title: str = "Elements", output_format: str = "text"
) -> None:
