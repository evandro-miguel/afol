...
| `just setup-uv` | `.agents/agents hydrate-uv` |
| `just lint-runtime` | `.agents/runtime/.venv/bin/ruff check .agents/runtime` |
| `just test-runtime` | `.agents/runtime/.venv/bin/python -m pytest .agents/runtime/tests` |
...
```justfile
just setup-uv     # Project-local uv setup
just setup        # Project-local virtualenv setup
...
`.agents/tools/uv/bin/uv --locked`. Normal validation runs from the project-local
virtualenvs directly.
