...
const totals = parseBunTextCoverage(`${result.stdout ?? ""}\n${result.stderr ?? ""}`);
const passed = totals.lines >= THRESHOLD && totals.functions >= THRESHOLD;

console.log(`coverage lines: ${formatPercent(totals.lines)}% (threshold ${THRESHOLD}%)`);
console.log(`coverage functions: ${formatPercent(totals.functions)}% (threshold ${THRESHOLD}%)`);
