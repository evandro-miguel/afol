...
function objectAt(value: unknown, key: string): Record<string, unknown> | null {
  ...
  return next !== null && typeof next === "object" && !Array.isArray(next) ? (next as Record<string, unknown>) : null;
}
...
export function normalizeProjectRelativePath(path: string, fallback: string): string {
...
function fromConfig(config: SchemaObject, path: string[], fallback: string): string {
  return normalizeProjectRelativePath(stringAt(config, path) ?? fallback, fallback);
}
...
export function resolveProjectPaths(root: string): ResolvedProjectPaths {
  ...
  const paths: ProjectPathConfig = {
    ...
    skillsDir: normalizeProjectRelativePath(
      stringAt(config, ["paths", "skills_dir"]) ?? stringAt(config, ["skills_sync", "project_dir"]) ?? `${agentsDir}/skills`,
      `${agentsDir}/skills`,
    ),
    wbDir,
    activeSessionFile: fromConfig(config, ["paths", "active_session_file"], ".afol/wb/.active_session"),
    ...
    mutationsDir,
    mutationBackupsDir: fromConfig(config, ["paths", "mutation_backups_dir"], `${mutationsDir}/backups`),
    mutationArchivesDir: fromConfig(config, ["paths", "mutation_archives_dir"], `${mutationsDir}/archives`),
    lockFile: fromConfig(config, ["paths", "lock_file"], `${agentsDir}/lock.json`),
    manifestFile: fromConfig(config, ["paths", "manifest_file"], `${agentsDir}/manifest.json`),
  };

  return { ...paths, abs: absolute(root, paths) };
}

export function readJsonObjectIfExists(path: string): Record<string, unknown> | null {
  if (!existsSync(path)) {
    return null;
  }
