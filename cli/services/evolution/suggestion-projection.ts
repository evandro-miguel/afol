					...
					db
						.query(
							`WITH ranked AS (
								...
								FROM observations
								WHERE project_id = ? AND fingerprint IN (${fingerprints.map(() => "?").join(",")})
							)
							SELECT * FROM ranked WHERE candidate_rank <= ${MAX_OBSERVATIONS_PER_CLUSTER}`,
						)
						.all(projectId, ...fingerprints) as Array<Record<string, unknown>>
				).map(observationRecordFromRow);
	const observationsByFingerprint = new Map<string, ObservationRecord[]>();
	for (const observation of observations) {
		const rows = observationsByFingerprint.get(observation.fingerprint) ?? [];
