...
import { type RuleEntry, listRules } from "../catalog/rules";
import { type SkillEntry, listSkills } from "../catalog/skills";
...
const INDEX_DIR = [".agents", "data", "index"] as const;
const RULES_INDEX_PATH = [...INDEX_DIR, "rules.json"] as const;
const SKILLS_INDEX_PATH = [...INDEX_DIR, "skills.json"] as const;
const SPECS_INDEX_PATH = [...INDEX_DIR, "specs.json"] as const;
const FILES_INDEX_PATH = [...INDEX_DIR, "files.json"] as const;

const FILE_INDEX_EXCLUDED_DIR_SEGMENTS = new Set([".git", ".gitnexus", ".ruff_cache", "node_modules", "coverage", "dist"]);

const ZERO_TIME = new Date(0).toISOString();
const FRESHNESS_CLOCK_SKEW_MS = 1_000;

function resolveDataIndexPath(root: string, parts: readonly string[]): string {
...
function isDirectoryExcluded(projectRoot: string, directoryPath: string): boolean {
  const normalizedPath = toRelativeProjectPath(projectRoot, directoryPath).replace(/\\/g, "/");

  if (normalizedPath === ".") {
    return false;
  }

  if (normalizedPath === ".agents/data/index" || normalizedPath.startsWith(".agents/data/index/")) {
    return true;
  }

  if (
    normalizedPath.startsWith(".agents/") &&
    normalizedPath !== ".agents/rules" &&
    !normalizedPath.startsWith(".agents/rules/") &&
    normalizedPath !== ".agents/skills" &&
  ...
  return segments.some((segment) => FILE_INDEX_EXCLUDED_DIR_SEGMENTS.has(segment));
}
...
function collectFilesUnder(
  ...
  while (stack.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isFile() && predicate(entry, relativePath, entryPath)) {
        out.push(entryPath);
      }
    }
  }
...
function collectSpecFiles(projectRoot: string): string[] {
  return collectFilesUnder(projectRoot, join("docs", "arc", "SPECS"), (_entry, relativePath) => relativePath.endsWith(".md"));
}
...
function latestRulesSource(root: string): number {
  ...
  const sourcePaths = collectSourceFiles([rulesRoot, join(rulesRoot, "index.json"), ...rulePaths]);
...
function latestSkillsSource(root: string): number {
  ...
  const skillPaths = skills
    .flatMap((skill: SkillEntry) => [join(root, skill.path), dirname(join(root, skill.path))])
    .concat(skillsRoot);
...
export function rebuildRulesIndex(root: string): RulesIndexSnapshot {
  ...
  const indexPath = resolveDataIndexPath(root, RULES_INDEX_PATH);
...
export function validateRulesIndex(root: string): ValidationResult {
  ...
  return validateSnapshot(snapshot, indexPath, "rules index", latestRulesSource(root));
}
...
export function rebuildSkillsIndex(root: string): SkillsIndexSnapshot {
  ...
  const indexPath = resolveDataIndexPath(root, SKILLS_INDEX_PATH);
  return writeSnapshot(indexPath, snapshot);
}
...
export function validateSkillsIndex(root: string): ValidationResult {
  ...
  if (
    ...
    return { ok: false, message: `invalid skills index snapshot: ${indexPath}` };
  }

  return validateSnapshot(snapshot, indexPath, "skills index", latestSkillsSource(root));
}
...
export function rebuildSpecsIndex(root: string): SpecsIndexSnapshot {
  ...
  const indexPath = resolveDataIndexPath(root, SPECS_INDEX_PATH);
  return writeSnapshot(indexPath, snapshot);
}
...
export function validateSpecsIndex(root: string): ValidationResult {
  ...
  return validateSnapshot(snapshot, indexPath, "specs index", latestSpecsSource(root));
}
...
export function rebuildFilesIndex(root: string): FilesIndexSnapshot {
  ...
  const indexPath = resolveDataIndexPath(root, FILES_INDEX_PATH);
  return writeSnapshot(indexPath, snapshot);
}
...
export function validateFilesIndex(root: string): ValidationResult {
  ...
  return validateSnapshot(snapshot, indexPath, "files index", latestFilesSource(root));
}
