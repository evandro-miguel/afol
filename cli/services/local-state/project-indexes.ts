...
function isDirectoryExcluded(projectRoot: string, directoryPath: string): boolean {
  ...
  const pathMatches = (base: string): boolean => normalizedPath === base || normalizedPath.startsWith(`${base}/`);
  const allowedOperationalRoots = [projectPaths.rulesDir, projectPaths.skillsDir];
