...
function normalizeRule(raw: RawRule): RuleEntry | null {
  const id = typeof raw.id === "string" ? raw.id.trim().toUpperCase() : "";
  if (!id) {
    return null;
  }
  const rawPath = typeof raw.path === "string" ? raw.path.trim() : `${id}.md`;
  const path = rawPath.startsWith(".agents/rules/") ? rawPath : `.agents/rules/${rawPath}`;
  return {
    id,
...
function fallbackRules(projectRoot: string): RuleEntry[] {
...
export function listRules(projectRoot: string): RuleEntry[] {
  const indexPath = join(projectRoot, ".agents", "rules", "index.json");
  if (!existsSync(indexPath)) {
    return fallbackRules(projectRoot);
  }
