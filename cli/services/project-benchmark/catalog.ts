...
const PROJECT_BENCHMARK_TEMPLATE_PREFIX = ".afol/adm/project-benchmarks/";
...
function parseTemplateJsonFile<T>(
	...
	const templateFile = DEFAULT_TEMPLATE_FILES[file];
	if (!templateFile) {
		issues.push({
			severity: "error",
...
function listBuiltinProjectFiles(): string[] {
	return Object.keys(DEFAULT_TEMPLATE_FILES)
		.filter(
			(file) =>
