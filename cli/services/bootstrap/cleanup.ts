...
const LEGACY_FILE_NAMES = ["uv.lock", "pyproject.toml", ".python-version"] as const;
...
function isLegacyExactFile(path: string): boolean {
  return LEGACY_EXACT_FILES.includes(path as (typeof LEGACY_EXACT_FILES)[number]);
}
...
export function planBootstrapCleanup(targetRoot: string): BootstrapCleanupPlan {
  ...
  return {
    candidates: candidates.map((path) => ({ path, reason: cleanupReason(path) })),
  };
}

export function cleanupBootstrapObsolete(targetRoot: string, candidates: readonly BootstrapCleanupCandidate[]): void {
