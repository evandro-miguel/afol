...
function resolveJournalPath(projectRoot: string): string {
  return resolve(projectRoot, ".agents", "data", "mutations", "mutations.jsonl");
}
...
function parseRecord(raw: string): MutationRecord | null {
  try {
    ...
    const statusValue = record.status === "applied" || record.status === "noop" ? record.status : null;
    if (statusValue === null) {
      return null;
    }
    const dryRun = record.dryRun === true || record.dryRun === false ? record.dryRun : false;
    ...
    const destinationPath = typeof record.destinationPath === "string" ? record.destinationPath : undefined;
    ...
    if (
      ...
      if (kind === "undo" && (statusValue === "applied" || statusValue === "noop")) {
      if (typeof record.targetMutationId !== "string" || destinationPath === undefined) {
        return null;
      }
      return {
        id,
      ...
      return {
        ...
        overwrittenBackupPath:
          typeof record.overwrittenBackupPath === "string" ? record.overwrittenBackupPath : null,
        beforeExisted: record.beforeExisted === true || record.beforeExisted === false ? record.beforeExisted : undefined,
        destinationExisted: record.destinationExisted === true || record.destinationExisted === false
...
export function appendMutationRecord(projectRoot: string, record: MutationRecord): void {
...
export function findMutationById(projectRoot: string, mutationId: string): MutationRecord | null {
  const records = loadMutationJournal(projectRoot);
  for (let index = records.length - 1; index >= 0; index -= 1) {
    const record = records[index];
    if (!record) {
