...
function createHealthyFixture(): string {
	...
	writeFileSync(join(root, ".afol", "pstr", "index.json"), JSON.stringify({
		kind: "pstr_index_v1",
		version: 1,
		generated_at: now,
		source: { project_root: root, pstr_dir: join(root, ".afol", "pstr") },
		maps: [
			{ id: "cli", scope: "cli", status: "current", authority: "observed", source_paths: ["cli/main.ts"], source_hash: "hash-1", file_count: 1, updated_at: now, stale_after: future, tags: ["pstr"] },
			{ id: "template", scope: "template", status: "current", authority: "observed", source_paths: ["src/project-template/index.ts"], source_hash: "hash-2", file_count: 1, updated_at: now, stale_after: future, tags: ["pstr"] },
			{ id: "docs", scope: "docs", status: "current", authority: "observed", source_paths: ["docs/readme.md"], source_hash: "hash-3", file_count: 1, updated_at: now, stale_after: future, tags: ["pstr"] },
			{ id: "config", scope: "config", status: "current", authority: "observed", source_paths: [".agents/config.json"], source_hash: "hash-4", file_count: 1, updated_at: now, stale_after: future, tags: ["pstr"] },
		],
	}) + "\n", "utf8");
	return root;
}
...
describe("sweep command", () => {
	test("daily sweep returns maintenance actions", async () => {
		...
		try {
			...
			const payload = JSON.parse(out[0] ?? "{}") as { checked: number; issues: number; actions: string[] };
