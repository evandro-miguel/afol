...
function mkRoot(): string {
  ...
  writeFileSync(join(root, ".agents", "rules", "RULE-001-tool-discovery.md"), "# Rule 1\n", "utf8");
  writeFileSync(join(root, ".agents", "rules", "RULE-004-validation-linting.md"), "# Rule 4\n", "utf8");
  return root;
}
...
describe("rule command", () => {
  ...
  test("resolves rules by surface and work type", async () => {
    ...
    try {
      ...
      expect(await runRuleCommand(["resolve", "--surface", "testing", "--work-type", "validation"], root, output.io)).toBe(0);
