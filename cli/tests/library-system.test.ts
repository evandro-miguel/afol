...
describe("library crud", () => {
	...
	test("proposeTopic creates topic with sources", () => {
		...
		try {
			...
			expect(created).toMatchObject({
				slug: "alpha-one",
				title: "Alpha One",
				sources: [{ id: "src-1", url: "http://example.com/alpha", title: "Alpha source", accessed_at: "2026-06-13T00:00:00.000Z" }],
				claims: [],
				tags: [],
			})
	...
	test("proposeTopic rejects missing accessed_at", () => {
		const root = createFixture()
		try {
			expect(() => proposeTopic(root, "alpha", "Alpha", [{ ...source(), accessed_at: "" }])).toThrow(
				"Every source requires id, url, title, and accessed_at.",
			)
		} finally {
			rmSync(root, { recursive: true, force: true })
		}
	...
	test("addSource adds source to existing topic", () => {
		...
		try {
			...
			addSource(root, "alpha", source({ id: "src-2", url: "http://example.com/beta", title: "Beta source" }))
			const updated = addSource(root, "alpha", source({ id: "src-2", url: "http://example.com/beta", title: "Beta source v2" }))
			expect(updated.sources).toHaveLength(2)
			expect(updated.sources[1]).toMatchObject({ id: "src-2", title: "Beta source v2" })
		} finally {
			rmSync(root, { recursive: true, force: true })
		}
	})
	...
	test("addClaim adds claim with valid source_ids", () => {
		...
		try {
			...
			const replaced = addClaim(root, "alpha", claim({ text: "Alpha claim updated" }))
			expect(updated.claims).toHaveLength(1)
			expect(replaced.claims).toHaveLength(1)
			expect(replaced.claims[0]).toMatchObject({ id: "claim-1", text: "Alpha claim updated", source_ids: ["src-1"] })
		} finally {
			rmSync(root, { recursive: true, force: true })
		}
	})
	...
	test("invalidateClaim on unknown claim throws", () => {
		...
		try {
			proposeTopic(root, "alpha", "Alpha", [source()])
			expect(() => invalidateClaim(root, "alpha", "missing", "wrong")).toThrow("Claim not found: missing")
		} finally {
			rmSync(root, { recursive: true, force: true })
		}
	})
	...
	test("searchLibrary finds matching claims", () => {
		...
		try {
			proposeTopic(root, "alpha", "Alpha", [source()])
			addClaim(root, "alpha", claim({ text: "The alpha claim mentions coverage" }))
	...
	test("rebuildLibraryIndex creates INDEX.json", () => {
		...
		try {
			...
			expect(snapshot).toMatchObject({
				...
				version: 1,
				topics: [{ slug: "alpha", title: "Alpha", source_count: 1, claim_count: 1 }],
			})
			const index = JSON.parse(readFileSync(join(root, ".afol", "library", "INDEX.json"), "utf8")) as {
				kind: string
				version: number
...
describe("library command", () => {
	...
	test("afol library list --json returns JSON", async () => {
		...
		try {
			...
			expect(JSON.parse(out.stdout[0] ?? "{}")).toMatchObject({ ok: true, topics: [] })
		} finally {
			rmSync(root, { recursive: true, force: true })
		}
	})
	...
	test('afol library propose --topic x --title "X" --url http://... --json creates topic', async () => {
		...
		try {
			const out = capture()
			expect(
				await runLibraryCommand("propose", ["--topic", "x", "--title", "X", "--url", "http://example.com/x", "--json"], root, out.io),
			).toBe(0)
			const payload = JSON.parse(out.stdout[0] ?? "{}") as { ok: boolean; topic: { slug: string; title: string; sources: Array<{ accessed_at: string }> } }
	...
	test('afol library topic --topic x --json returns topic details', async () => {
		...
		try {
			...
			addClaim(root, "x", claim({ id: "claimId", text: "text", source_ids: ["src1"] }))
			const out = capture()
			expect(await runLibraryCommand("topic", ["--topic", "x", "--json"], root, out.io)).toBe(0)
			const payload = JSON.parse(out.stdout[0] ?? "{}") as { ok: boolean; topic: { slug: string; claims: Array<{ id: string }> } }
			expect(payload.ok).toBe(true)
			expect(payload.topic.slug).toBe("x")
	...
	test('afol library add-claim --topic x --claim "text" --source src1 --json adds claim', async () => {
		...
		try {
			...
			const out = capture()
			expect(
				await runLibraryCommand("add-claim", ["--topic", "x", "--claim", "text", "--source", "src1", "--json"], root, out.io),
			).toBe(0)
			const payload = JSON.parse(out.stdout[0] ?? "{}") as { ok: boolean; topic: { claims: Array<{ text: string }> } }
			expect(payload.ok).toBe(true)
			expect(payload.topic.claims).toHaveLength(1)
	...
	test('afol library add-source --topic x --url http://... --json adds source', async () => {
		...
		try {
			...
			const out = capture()
			expect(
				await runLibraryCommand("add-source", ["--topic", "x", "--url", "http://example.com/beta", "--title", "Beta", "--json"], root, out.io),
			).toBe(0)
			const payload = JSON.parse(out.stdout[0] ?? "{}") as { ok: boolean; topic: { sources: Array<{ id: string; title: string }> } }
			expect(payload.ok).toBe(true)
	...
	test('afol library rebuild-index --json returns snapshot', async () => {
		...
		try {
			proposeTopic(root, "x", "X", [source({ id: "src1" })])
			const out = capture()
			expect(await runLibraryCommand("rebuild-index", ["--json"], root, out.io)).toBe(0)
			const payload = JSON.parse(out.stdout[0] ?? "{}") as { ok: boolean; snapshot: { topics: Array<{ slug: string }> } }
			expect(payload.ok).toBe(true)
	...
	test('afol library search without query returns error exit code', async () => {
		...
		try {
			...
			expect(out.stderr.join("\n")).toContain("Missing --query for library search.")
		} finally {
			rmSync(root, { recursive: true, force: true })
		}
	})
	...
	test('afol library add-claim without source returns error exit code', async () => {
		...
		try {
			proposeTopic(root, "x", "X", [source({ id: "src1" })])
			const out = capture()
			expect(await runLibraryCommand("add-claim", ["--topic", "x", "--claim", "text"], root, out.io)).toBe(2)
			expect(out.stderr.join("\n")).toContain("Missing --source for library add-claim.")
		} finally {
			rmSync(root, { recursive: true, force: true })
		}
	})
	...
	test('afol library search --query text --json returns results', async () => {
		...
		try {
			...
			const out = capture()
			expect(await runLibraryCommand("search", ["--query", "text", "--json"], root, out.io)).toBe(0)
			const payload = JSON.parse(out.stdout[0] ?? "{}") as { ok: boolean; matches: Array<{ topic: { slug: string } }> }
			expect(payload.ok).toBe(true)
	...
	test('afol library invalidate --topic x --claim claimId --reason "wrong" --json invalidates', async () => {
		...
		try {
			...
			const out = capture()
			expect(
				await runLibraryCommand("invalidate", ["--topic", "x", "--claim", "claimId", "--reason", "wrong", "--json"], root, out.io),
			).toBe(0)
			const payload = JSON.parse(out.stdout[0] ?? "{}") as { ok: boolean; topic: { claims: Array<{ status: string; invalidated_reason: string }> } }
			expect(payload.ok).toBe(true)
			expect(payload.topic.claims[0]).toMatchObject({ status: "invalidated", invalidated_reason: "wrong" })
		} finally {
			rmSync(root, { recursive: true, force: true })
		}
	})
	...
	test("afol library propose without topic returns error exit code", async () => {
		...
		try {
			const out = capture()
			expect(await runLibraryCommand("propose", ["--title", "X", "--url", "http://example.com/x"], root, out.io)).toBe(2)
			expect(out.stderr.join("\n")).toContain("Missing --topic for library propose.")
		} finally {
			rmSync(root, { recursive: true, force: true })
		}
	})
	...
	test("afol library topic without slug returns error exit code", async () => {
		...
		try {
			...
			expect(out.stderr.join("\n")).toContain("Missing --topic for library topic.")
		} finally {
			rmSync(root, { recursive: true, force: true })
		}
	})
	...
	test("afol library topic not found returns 1", async () => {
		...
		try {
			const out = capture()
			expect(await runLibraryCommand("topic", ["--topic", "missing"], root, out.io)).toBe(1)
			expect(out.stderr.join("\n")).toContain("Library topic not found: missing")
		} finally {
			rmSync(root, { recursive: true, force: true })
		}
	})
