...
describe("evolve external import commands", () => {
	...
	test("confirmation persists redacted import and list is read-only", async () => {
		const { root, source } = fixture();
		try {
			...
			expect(
				await runEvolveCommand("external", ["list", "--json"], root, listed.io),
			).toBe(0);
	...
	test("external list fails closed for stale or symlinked databases", async () => {
		const { root } = fixture();
		try {
			...
			const stale = capture();
			expect(
				await runEvolveCommand("external", ["list", "--json"], root, stale.io),
			).toBe(2);
			...
			expect(
				await runEvolveCommand("external", ["list", "--json"], root, linked.io),
			).toBe(2);
		} finally {
			rmSync(root, { recursive: true, force: true });
		}
	...
	test("two processes confirm the same import exactly once", async () => {
		const { root, source } = fixture();
		try {
