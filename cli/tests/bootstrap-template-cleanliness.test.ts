...
import { DEFAULT_TEMPLATE_FILES, DEFAULT_TEMPLATE_METADATA } from "../generated/template";
...
describe("generated template cleanliness", () => {
  test("generated payload excludes forbidden paths", () => {
    ...
    const forbidden = paths.filter((path) => matchesTemplateForbiddenPattern(path));

    expect(DEFAULT_TEMPLATE_METADATA.excludedForbiddenCount).toBe(0);
    expect(DEFAULT_TEMPLATE_METADATA.generatedAt).toBe("1970-01-01T00:00:00.000Z");
    expect(forbidden).toEqual([]);
    expect(paths.some((path) => path.startsWith(".agents/scripts/"))).toBe(false);
