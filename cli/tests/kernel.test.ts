...
describe("kernel front-door", () => {
	...
	test("legacy-prefixed delegate commands still route through adapter", () => {
		const script = "#!/usr/bin/env bash\necho ARGS:$*";
		const root = mkProjectRoot("legacy-delegate", script);
		try {
			const delegateCases: { args: string[]; expected: string }[] = [
				{ args: ["legacy:t", "list"], expected: "ARGS:task list" },
				{
					args: ["legacy:inspect-target", "--repo-root", "/tmp/project"],
					expected: "ARGS:inspect-target --repo-root /tmp/project",
				},
				{
					args: ["legacy:adoption-plan", "--repo-root", "/tmp/project"],
					expected: "ARGS:adoption-plan --repo-root /tmp/project",
				},
			];
	...
	test("init forwards confirmed provider-compatible mutable cleanup to bootstrap", () => {
		...
		try {
			...
			const archives = readdirSync(join(root, ".agents", "z-arq"));
			expect(archives).toHaveLength(1);
			const archiveRoot = join(root, ".agents", "z-arq", archives[0] ?? "");
	...
	test("preserves legacy exit code through adapter", () => {
		...
		try {
			const proc = runKernel(root, ["legacy:task"]);
	...
	test("delegate command in TypeScript template fails without missing wrapper lookup", () => {
		...
		try {
			const proc = runKernel(root, ["legacy:task", "list"]);

			expect(proc.status).toBe(127);
			expect(proc.stdout as string).toBe("");
			expect(proc.stderr as string).toContain(
				"err delegate-unavailable command=task",
			);
			expect(proc.stderr as string).toContain(
				"not available in TypeScript template",
			);
