...
function runBootstrap(target: string, args: string[] = []): ReturnType<typeof spawnSync> {
...
function writeLegacyTargetFixture(targetRoot: string): void {
  ...
  writeFileSync(join(agentsRoot, "scripts", "bootstrap.py"), "print('legacy script')", "utf8");
  writeFileSync(join(runtimeCache, "runtime.py"), "print('runtime shim')", "utf8");
  writeFileSync(join(agentsRoot, "agents"), "# legacy adapter wrapper", "utf8");
  writeFileSync(join(agentsRoot, "agents-mcp"), "# legacy adapter wrapper", "utf8");
  writeFileSync(join(legacyPyRoot, "uv.lock"), "legacy uv lock", "utf8");
  writeFileSync(join(legacyPyRoot, "pyproject.toml"), "[project]\\nname = 'legacy'\\n", "utf8");
...
function assertLegacyArtifactsRemoved(targetRoot: string): void {
  ...
  expect(existsSync(join(targetRoot, ".agents", "legacy", "uv.lock"))).toBe(false);
  expect(existsSync(join(targetRoot, ".agents", "legacy", "pyproject.toml"))).toBe(false);
  expect(existsSync(join(targetRoot, ".agents", "legacy", ".python-version"))).toBe(false);
  expect(existsSync(join(targetRoot, ".agents", "legacy", ".venv"))).toBe(false);
  expect(existsSync(join(targetRoot, ".agents", "legacy", ".venv", "__pycache__"))).toBe(false);
}
...
describe("bootstrap cleanup of legacy Python scaffold", () => {
  test("dry-run reports legacy cleanup and preserves files", () => {
    ...
    try {
      ...
      expect((proc.stdout as string)).toContain("cleanup-pending .agents/scripts legacy-script-root");
      expect((proc.stdout as string)).toContain("cleanup-pending .agents/runtime legacy-runtime-root");
      ...
      expect(existsSync(join(target, ".agents", "legacy", "uv.lock"))).toBe(true);
      expect(existsSync(join(target, ".agents", "legacy", "pyproject.toml"))).toBe(true);
      expect(existsSync(join(target, ".agents", "legacy", ".python-version"))).toBe(true);
      expect(existsSync(join(target, ".agents", "legacy", ".venv"))).toBe(true);
      expect(existsSync(join(target, ".agents", "legacy", ".venv", "__pycache__"))).toBe(true);
      expect(existsSync(join(target, ".agents", "keep", "project-owned.md"))).toBe(true);
    } finally {
  ...
  test("apply removes legacy artifacts only with --cleanup-obsolete", () => {
    ...
    try {
      ...
      expect(cleanupProc.stdout).toContain("cleanup-removed .agents/scripts legacy-script-root");
      expect(cleanupProc.stdout).toContain("cleanup-removed .agents/runtime legacy-runtime-root");
