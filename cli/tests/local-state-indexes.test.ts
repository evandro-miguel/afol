...
function buildFixture() {
  ...
  writeFileSync(
    join(rulesDir, "RULE-200.md"),
    "# Rule 200\n",
    "utf8",
  );
  writeFileSync(
    join(rulesDir, "RULE-010.md"),
    "# Rule 010\n",
    "utf8",
  );
...
describe("local-state project indexer", () => {
  test("rebuildProjectIndexes builds ordered snapshots and omits .agents/data/index", () => {
    ...
    try {
      ...
      expect(snapshot.skills.skills.map((skill) => skill.name)).toEqual(["alpha skill", "beta skill"]);
      ...
      const parsedRulesSnapshot = JSON.parse(readFileSync(rulesPath, "utf8")) as { rules: { id: string }[] };
      expect(parsedRulesSnapshot.rules.map((rule) => rule.id)).toEqual(["RULE-010", "RULE-200"]);

      const filesPath = join(root, ".agents", "data", "index", "files.json");
      const filesSnapshot = JSON.parse(readFileSync(filesPath, "utf8")) as FilesIndexSnapshot;
      const sorted = filesSnapshot.files.map((file) => file.path).sort((a, b) => a.localeCompare(b));
      ...
      const skillsSnapshot = JSON.parse(readFileSync(skillsPath, "utf8")) as SkillsIndexSnapshot;
      const specsSnapshot = JSON.parse(readFileSync(specsPath, "utf8")) as SpecsIndexSnapshot;

      const orderedSkills = skillsSnapshot.skills.map((skill) => skill.name).sort((a, b) => a.localeCompare(b));
      expect(skillsSnapshot.skills.map((skill) => skill.name)).toEqual(orderedSkills);
      ...
      writeFileSync(join(root, ".agents", "tmp", "scratch.txt"), "ignore", "utf8");
      const blockedSnapshot = rebuildFilesIndex(root);
      expect(blockedSnapshot.files.some((entry) => entry.path === ".agents/data/index/ignore.txt")).toBe(false);
      expect(blockedSnapshot.files.some((entry) => entry.path === ".agents/tmp/scratch.txt")).toBe(false);
    } finally {
