...
function requireWriteContext(args: CommandArgs): void {
  if (!args.dryRun && (!args.session.trim() || !args.taskId.trim() || !args.reason.trim())) {
    throw new Error("Real file mutation requires --session, --task-id, and --reason.");
  }
}
...
function resolveSafePath(projectRoot: string, targetPath: string): { path: string; relativePath: string } {
...
function archiveDestination(projectRoot: string, mutationId: string, relativePath: string): { path: string; relativePath: string } {
  const safe = sanitizeForFilename(relativePath);
  const relative = join(resolveProjectPaths(projectRoot).mutationArchivesDir, `${mutationId}-${safe}`);
...
function applyProjectMutationDefaults<T extends CommandArgs>(args: T, projectRoot: string): T {
  const defaults = projectMutationDefaults(projectRoot);
  const next = { ...args };
  if (next.path === DEFAULT_PATCH_PATH || next.path === DEFAULT_MOVE_SOURCE) {
    next.path = next.command === "mv" ? defaults.moveSource : defaults.patchPath;
  }
  if (next.command === "mv" && "destinationPath" in next && next.destinationPath === DEFAULT_MOVE_DESTINATION) {
...
function parseMoveArgs(values: string[]): MoveArgs {
  ...
  const destination = parsed.destinationArg
...
function runPatchMutation(args: PatchArgs, projectRoot: string): CommandResult {
  ...
  if (existsSync(resolved.path)) {
    backupPathValue = backupPath(projectRoot, mutationId, resolved.relativePath);
...
function runMoveMutation(args: MoveArgs, projectRoot: string): CommandResult {
  ...
  if (destinationExisted) {
    overwrittenBackupPath = backupPath(projectRoot, mutationId, `${destination.relativePath}.overwritten`);
...
function undoPatchMutation(args: CommandArgs, mutation: MutationRecord, projectRoot: string): CommandResult {
  ...
  if (args.dryRun) {
    ...
    const after = backupPath && existsSync(backupPath) ? readTextOrEmpty(backupPath) : "";
    return {
      command: "ud",
      status: "dry-run",
      dry_run: true,
...
function undoMoveMutation(args: CommandArgs, mutation: MutationRecord, projectRoot: string): CommandResult {
  if (mutation.kind !== "move" || !mutation.sourcePath || !mutation.destinationPath) {
  ...
  if (args.dryRun) {
    ...
    const afterSource = existsSync(destination.path) ? readTextOrEmpty(destination.path) : "";
    const afterDestination = mutation.overwrittenBackupPath && existsSync(mutation.overwrittenBackupPath)
    ...
    return {
      ...
      diff_preview: makeDiffPreview(beforeDestination, afterDestination, mutation.destinationPath),
    };
  }
  ...
  const beforeSource = existsSync(source.path) ? readTextOrEmpty(source.path) : "";
  const beforeDestination = existsSync(destination.path) ? readTextOrEmpty(destination.path) : "";
  const beforeSourceHash = beforeSource.length > 0 ? normalizeHash(beforeSource) : null;
  ...
  if (mutation.overwrittenBackupPath && existsSync(mutation.overwrittenBackupPath)) {
    mkdirSync(dirname(destination.path), { recursive: true });
  ...
  const afterDestination = existsSync(destination.path) ? readTextOrEmpty(destination.path) : "";
  const mutationId = createMutationId();
...
function runUndoMutation(args: UndoArgs, projectRoot: string): CommandResult {
  ...
  if (!target) {
    return {
      ...
      message: args.mutationId ? `No mutation found for id ${args.mutationId}` : "No supported mutation found for session/task",
    };
  }
  ...
  if (args.mutationId && (target.session !== args.session || target.taskId !== args.taskId)) {
    throw new Error(
...
function formatHumanResult(result: CommandResult): string {
  const lines = [
    ...
    result.target_mutation_id ? `target_mutation_id=${result.target_mutation_id}` : "",
    result.message || "",
  ].filter(Boolean);
...
function outputResult(result: CommandResult, io: CommandIo, asJson: boolean): void {
  if (asJson) {
...
export async function runFileCommand(
  ...
  try {
    ...
    if (rawCommand === "pt" || rawCommand === "patch") {
      const parsed = applyProjectMutationDefaults(parsePatchArgs(rest), projectRoot);
      asJson = parsed.json;
      result = runPatchMutation(parsed, projectRoot);
    } else if (rawCommand === "mv" || rawCommand === "move") {
      const parsed = applyProjectMutationDefaults(parseMoveArgs(rest), projectRoot);
      asJson = parsed.json;
      ...
      const parsed = applyProjectMutationDefaults(parseArchiveArgs(rest), projectRoot);
      asJson = parsed.json;
