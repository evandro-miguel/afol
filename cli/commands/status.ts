...
const FIELD_HEADERS = ["STATUS", "TASK", "FILES_WRITTEN", "VALIDATION_OR_CHECKS", "BLOCKERS", "NEXT"];
...
function extractFieldList(content: string, label: "FILES_WRITTEN" | "VALIDATION_OR_CHECKS" | "BLOCKERS" | "NEXT"): string[] {
...
function extractTaskId(content: string, frontmatter: Record<string, string>, fileName: string): string {
...
function pickTaskFile(projectRoot: string, activeSession: string): string | null {
  const sessionDir = join(projectRoot, ".agents", "wb", activeSession);
...
function readStatusSnapshot(projectRoot: string): StatusSnapshot {
  ...
  const lockPath = join(loaded.value.root, ".agents", "lock.json");
  const activeSessionPath = join(loaded.value.root, ".agents", "wb", ".active_session");
  ...
  const status = extractTaskState(content, task) ?? frontmatter.status?.trim() ?? "none";
