...
import { findSkill, listSkills, searchSkills } from "../services/catalog/skills";
...
export async function runRuleCommand(
  ...
  try {
    ...
    if (command === "list" || command === "ls") {
      ...
      io.stdout([`rules: ${rules.length}`, ...rules.map(formatRule)].join("\n"));
      return 0;
    }
    ...
    if (command === "resolve") {
      ...
      io.stdout([`resolved rules: ${rules.length}`, ...rules.map(formatRule)].join("\n"));
      return 0;
    }
...
export async function runSkillCommand(
  ...
  try {
    ...
    if (command === "list" || command === "ls") {
      ...
      io.stdout([`skills: ${skills.length}`, ...skills.map(formatSkill)].join("\n"));
      return 0;
    }
    if (command === "show" || command === "get") {
      const identifier = rest[0];
      ...
      io.stdout([`skill: ${skill.name}`, `path: ${skill.path}`, `description: ${skill.description || "none"}`].join("\n"));
      return 0;
    }
    if (command === "search") {
      ...
      io.stdout([`skill matches: ${matches.length}`, ...matches.map(formatSkill)].join("\n"));
      return 0;
    }
